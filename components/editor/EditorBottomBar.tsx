...
export function EditorBottomBar() {
  ...
  const setIsPageTransitioning = useEditorStore((s) => s.setIsPageTransitioning);
  ...
  return (
    <div className="flex-shrink-0 bg-white border-t border-gray-100 flex flex-col w-full relative z-[2000]">
      {/* Primary Toolbar */}
      <div className="bg-white/95 backdrop-blur-xl py-1 px-4 flex items-center justify-between shadow-[0_-5px_20px_rgba(0,0,0,0.02)] transition-all duration-300 h-10">
        ...
        <div className="flex items-center gap-4 flex-1 justify-center">
          <button
            onClick={() => currentSpreadIndex > 0 && handlePageSelect(currentSpreadIndex - 1)}
          ...
          <button
            onClick={() => currentSpreadIndex < spreads.length - 1 && handlePageSelect(currentSpreadIndex + 1)}
        ...
        <div className="flex items-center gap-3 flex-1 justify-end">
          <button
            ...
            className={`px-2 h-8 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${showThumbnails ? "bg-red-50 text-red-600" : "text-gray-400 hover:text-[#2d2d2d]"
              }`}
          >
          ...
          <div className="flex items-center gap-1 bg-gray-50/50 rounded-full p-1 transition-all duration-500 overflow-hidden">
            <button
              ...
              className={`w-8 h-8 flex items-center justify-center rounded-full transition-all active:scale-95 ${showPageMenu ? "bg-[#2d2d2d] text-white shadow-md rotate-90" : "text-gray-400 hover:text-gray-600"
                }`}
            >
            ...
            {showPageMenu && (
              <div className="flex items-center gap-1 px-1 animate-in slide-in-from-right-4 fade-in duration-300">
                ...
                {/* Delete: Disabled on locked pages for non-admin users */}
                <button
                  onClick={() => { removeSpread(currentSpreadIndex); setShowPageMenu(false); }}
                  disabled={spreads.length <= 1 || (spreads[currentSpreadIndex]?.isOriginalTemplate && !isAdmin)}
                  ...
                  title={spreads[currentSpreadIndex]?.isOriginalTemplate && !isAdmin ? "Template pages cannot be deleted" : "Delete Spread"}
                >
      ...
      {showThumbnails && (() => {
        ...
        return (
          <div className="relative border-t border-gray-100 bg-gray-50/20 pb-2">
            {/* Left Arrow */}
            <button
              onClick={scrollLeft}
            ...
            <div
              ref={scrollRef}
              className="flex items-center gap-3 px-14 py-2 overflow-x-auto scroll-smooth"
...
const MiniPage = memo(({ page, isLeft }: { page: any; isLeft?: boolean }) => {
  ...
  if (bg) {
    if (bg === '/assets/layer-13.png') bg = '/assets/crix2.png';
    if (bg.includes('crix2.PNG')) bg = bg.replace('crix2.PNG', 'crix2.png');
    if (bg.includes('historyLayer.PNG')) bg = bg.replace('historyLayer.PNG', 'historyLayer.png');
    if (bg.includes('banner.PNG')) bg = bg.replace('banner.PNG', 'banner.png');
  }

  const bgStyle = bg?.startsWith("http") || bg?.startsWith("data:") || bg?.startsWith("/")
    ? { backgroundImage: `url(${bg})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : { backgroundColor: bg || "white" };

  return (
    <div
      className={`w-10 h-10 relative overflow-hidden shrink-0 ${!isLeft ? 'border-l border-gray-100' : ''}`}
      style={bgStyle}
    >
      <svg
        viewBox="0 0 500 500"
      ...
      >
        {page.elements?.map((el: any) => {
          ...
          if (el.type === "image" || el.type === "sticker" || el.type === "photo-card") {
            if (el.src) {
              // Apply the same patch used in EditorCanvas to prevent Vercel 404s
              let actualSrc = el.src;
              if (actualSrc === '/assets/layer-13.png') actualSrc = '/assets/crix2.png';
              if (actualSrc.includes('crix2.PNG')) actualSrc = actualSrc.replace('crix2.PNG', 'crix2.png');
              if (actualSrc.includes('historyLayer.PNG')) actualSrc = actualSrc.replace('historyLayer.PNG', 'historyLayer.png');
              if (actualSrc.includes('banner.PNG')) actualSrc = actualSrc.replace('banner.PNG', 'banner.png');
          ...
          if (el.type === "text") {
            const isPlaceholderText = !el.text || el.text === "Enter Text" || el.text === "Your Name" || el.text === "( Your Name )" || el.text === "Insert Your Name";
            const displayText = el.text || "Enter Text";
              ...
              return (
                <g key={el.id} transform={transform}>
                  <rect
                    ...
                    strokeWidth={el.strokeWidth || 2}
                  />
                  {el.type === "checkbox" && el.isChecked && (
                    <text
                      x={el.x + el.width / 2}
                      y={el.y + el.height / 2}
                      fontSize={el.width * 1.2}
